...
This schema is based on BIP-32{% include ref.html id="4" %} hierarchical deterministic derivations of public keys. With a seed you can create multiple private keys, that private keys control multiple identities. These identities, as far as the model is correctly designed, do not share information that can associate one with another.

The master key entropy is represented by a BIP-39 mnemonic phrase{% include ref.html id="5" %}. The derivation path used for RSK is `m/44'/137'/0'/0/i`{% include ref.html id="6" %} where `i` is used to index different identities.

![multi_identity_model](/assets/img/ssi/04_multi_identity_model.png)

> RIF Identity expresses support for _BIP-32_ and _BIP-39_
